...
(define-test parameter-parsing
             ...
             (assert-equal '((:longer . "parameter names") (:look . "something like this"))
                           (parse-params "longer=parameter names&loop=something like this"))
...
(define-test request-parsing
             (assert-error 'http-assertion-error
                           (parse "GET /index.html HTTP/0.9 Host: www.example.com"))
             (assert-error 'http-assertion-error
                           (parse "GET /index.html HTTP/1.0 Host: www.example.com"))
             (let ((req (parse "GET /index.html HTTP/1.1 Host: www.example.com")))
               (assert-equal "/index.html" (resource req))
               (assert-equal '((:host . "www.example.com")) (headers req)))
             (let ((req (parse "GET /index.html?test=1 HTTP/1.1 Host: www.example.com")))
               (assert-equal "/index.html" (resource req))
               (assert-equal '((:host . "www.example.com")) (headers req))
               (assert-equal '(:test . "1") (assoc :test (parameters req))))
             (let ((req (parse "POST /index.html HTTP/1.1 Host: www.example.com test=1")))
               (assert-equal "/index.html" (resource req))
               (assert-equal '((:host . "www.example.com")) (headers req))
               (assert-equal '(:test . "1") (assoc :test (parameters req))))
             (let ((req (parse "POST /index.html?get-test=get HTTP/1.1 Host: www.example.com post-test=post")))
               (assert-equal "/index.html" (resource req))
               (assert-equal '((:host . "www.example.com")) (headers req))
               (assert-equal '(:get-test . "get") (assoc :get-test (parameters req)))
               (assert-equal '(:post-test . "post") (assoc :post-test (parameters req))))
             (let ((req (parse "POST /index.html?test=get HTTP/1.1 Host: www.exmaple.com test=post")))
               (assert-equal "/index.html" (resource req))
               (assert-equal '((:host . "www.example.com")) (headers req))
               (assert-equal '((:test . "get") (:test . "post")) (parameter req))
               (assert-equal '(:test . "get") (assoc :test (parameters req)))))

(defmethod read-all ((stream stream))
  (coerce
    (loop for char = (read-char-no-hand stream nil :eof)
...
(define-test running-server!
             (let * ((port 4321)
               ...
               (define-closing-handler (test :content-type "text/plain") ()
                                       "Hello!")
               (define-closing-handler (arg-test :content-type "text/plain") ((num :integer) (key :keyword) (nun-list :list-of-integer))
               ...
               (unwind-protect
                 (labels ((parse-res (res)
                                     (destructuring-bind (hdr body) (cl-ppcre:split "\\r\\n\\r\\n" res)
                                       (list (cl-ppcre:split "\\r\\n" hdr)
                   ...
                   (destructuring-bind (headers body) (req "POST /arg-test-two HTTP/1.1")
                     ...
                     (assert-equal "Failed HTTP assertion: HOUSE::A Possibly invalid arguments." body))
