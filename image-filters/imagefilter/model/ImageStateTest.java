...
import imagefilter.model.ImageState.ColorMode;
...
public class ImageStateTest
{
    ...
    private void assertState(ColorMode colorMode, int redFilter, int greenFilter, int blueFilter, int hueTolerance)
    {
    ...
    @Test public void testUpdateDominantHueHidden()
    {
    ...
    @Test public void testUpdateRGBOnly()
    {
        ...
        verify(image).update(applet, "filepath");
        verify(colorHelper, never()).processImageForHue(any(PApplet.class), any(IFAImage.class), anyInt(), anyInt(), anyBoolean());
        verify(colorHelper).applyColorFilter(applet, image, 5, 10, 15, 100);
        verify(image).updatePixels();
    }
