    ...
    def send(self, sender, destinations, message):
        ...
        def sendto(dest, message):
            if dest == sender.address:
                ...
                self.set_timer(dest, delay, functools.partial(self.nodes[dest].receive, sender.address, message))
...
class Replica(Role):
    ...
    def commit(self, slot, proposal):
        decided_proposal = [p for s, p in self.decisions.items() if s < slot]
        if proposal in decided_proposal:
            self.logger.info(f'not committing duplicate proposal {proposal} at slot {slot}')
            return
...
class Requester(Role):
    cliend_ids = itertools.count(start=100000)

    def __init__(self, node, n, callback):
        super().__init__(node)
        self.client_id = next(self.cliend_ids)
        self.n = n
        self.output = None
        self.callback = callback
