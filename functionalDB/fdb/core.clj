...
(defn- update-attr-in-index [index ent-id attr-name target-val operation]
  (let [colled-target-val (collify target-val)
        update-entry-fn (fn [indx v1] (update-entry-in-index indx ((from-eav index) ent-id attr-name v1) operation))]
...
(defn add-entities [db ent-seq] (reduce add-entity db ent-seq))
